...
export async function installSkill(skillsDir?: string): Promise<string> {
  ...
  const configuredRoot = skillsDir ?? process.env.CODEX_HOME ?? resolve(homedir(), ".codex");
  const root = resolve(configuredRoot, skillsDir === undefined ? "skills" : "");
  const target = resolve(root, "narrated-code-review");
  await mkdir(dirname(target), { recursive: true });
  try {
    const existing = await lstat(target);
    if (existing.isSymbolicLink() && resolve(dirname(target), await readlink(target)) === source)
      return target;
