...
export function launchReviewPane(sessionPath: string, repoRoot: string): string {
  ...
  const surfacesResult = spawnSync("cmux", withWorkspace(["list-pane-surfaces", "--pane", pane]), { encoding: "utf8" });
  ...
  const sendResult = spawnSync("cmux", withWorkspace(["send", "--surface", surface, command]), { encoding: "utf8" });
